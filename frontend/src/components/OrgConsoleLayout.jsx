...
export default function OrgConsoleLayout() {
  ...
  // Владелец видит все разделы; приглашённая команда — без брендинга и подписки
  // (эти операции owner-only на бэкенде).
  const isOwner = ["org", "admin"].includes(user?.role);
  const navItems = NAV.filter((item) => isOwner || !item.ownerOnly);
        ...
        <nav className="con-sidebar" aria-label="Разделы консоли">
          ...
          <div className="con-user">
            ...
            <span className="con-user-tx">
              ...
              <span>Оргкомитет · {isOwner ? "Владелец" : "Команда"}</span>
            </span>
          </div>
        </nav>

        <main className="con-main">
          <Outlet context={{ org, setOrg, conference, setConference, isOwner }} />
        </main>
      </div>
    </div>
  );
